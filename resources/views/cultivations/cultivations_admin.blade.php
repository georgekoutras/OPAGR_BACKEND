                                ...
                                @foreach($cultivations as $cultivation)
                                    <tr>
                                        @if($cultivation->user()->get()->isNotEmpty())
                                            <td>{{$cultivation->user()->pluck('email')->first()}}</td>
                                            <td>{{$cultivation->user()->pluck('state')->first()}}</td>
                                            <td>
                                                <a href="/cultivations/details/{{$cultivation->name}}">{{$cultivation->name}}</a>
                                            </td>
                                            <td>{{$cultivation->deleted_at ? 'inactive' : 'active'}}</td>
                                            <td>
                                                <form method="post" action="{{route('cultivation.delete')}}">
                                                    @csrf
                                                    <input name="cultivation" value="{{$cultivation->name}}" hidden>
                                                    <button type="submit"
                                                            class="btn btn-outline-danger">{{$cultivation->deleted_at ? 'Activate' : 'Deactivate'}}</button>
                                                </form>
                                            </td>
                                        @endif
                                    </tr>
